...
Queues execute with an approval gate between items. Only the first item starts in a `queued` state (which the worker automatically claims). Subsequent items start in a `paused` state. The worker stops after completing an item and explicitly avoids claiming paused items. Users must manually advance the queue via `POST /v1/queues/:queueId/advance` (changes next paused item to queued), or they can skip an item (`POST /v1/queues/:queueId/skip`), or cancel the rest of the queue (`POST /v1/queues/:queueId/cancel`).

### Planning Mode
Jobs can run in `mode: "plan"` instead of `"execute"`. In this mode, the agent receives explicit instructions to decompose the prompt into a list of child jobs scoped to single repositories, writing them to a `PLAN.json` file in the sandbox workspace root. The worker parses this file, skips execution side-effects (like commits and PRs), and saves the plan in `result.plan`. The dashboard then displays the proposed plan with an "Approve & Enqueue" action, which POSTs the child tasks as a queue with the planning job as the `parentJobId`.
